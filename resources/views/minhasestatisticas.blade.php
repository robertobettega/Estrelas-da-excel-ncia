            ...
            @foreach ($qualidades_cards as $qualidades)
                <div class="card card-estatistica">
                    <div class="d-flex flex-wrap">
                        <div>
                            <img src={{ $qualidades->ICONES }} style="width: 160px; margin: 15px" alt="Segurança">
                            <div><b>{{ $qualidades->DESCRICAO }}</b></div>
                        </div>
                        <div class="row align-items-center">
                            @foreach ($countpin_cards as $count)
                                @if ($count->QUALIDADE_NOME == $qualidades->DESCRICAO)
                                    <div class="col" style="padding: 0px; margin-left: 5px"><b>{{ $count->TOTAL_QUALIDADE }}</b></div>
                                @endif
                            @endforeach
                            <img class="img-estat" src="images/estrela.png" style="padding: 0px">
                        </div>
                    </div>
                </div>
            @endforeach
                        ...
                        @foreach ($pinusuarios as $pin)
                            <div class="col-sm-3 mb-2 mb-sm-0" style="margin: 10px">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title">
                                            <i class="bi bi-star-fill"></i>{{ $pin->QUALIDADE_NOME }}
                                        </h5>
                                        ...
                                        <p class="card-text"><small class="text-body-secondary">Pin enviado em
                                                {{ $pin->DATA_PIN }} às {{ $pin->HORA_PIN }}</small></p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
